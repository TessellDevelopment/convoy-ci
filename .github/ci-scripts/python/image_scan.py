...
def post_to_slack(message):
    ...
    try:
        response = client.chat_postMessage(
            channel=os.environ.get("CHANNEL_ID"), text=full_message
        )
        return response["ok"]
...
print(
    f"Filesize of trivy_image_scan_result is {os.path.getsize('trivy_image_scan_result.json')}"
)
print(
    f"Filesize of dockle_image_scan_result is {os.path.getsize('dockle_image_scan_result.json')}"
)
...
headers = {"Content-Type": "application/json", "x-api-key": CONVOY_API_KEY}
